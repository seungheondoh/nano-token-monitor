...
type SourceKey = "include_claude" | "include_codex";
...
export function SourceSelector() {
  ...
  useEffect(() => {
    invoke<boolean>("is_codex_available").catch(() => false).then((codex) => {
      setCodexAvailable(codex);
      setAvailabilityLoaded(true);
    });
  }, []);

  // Reconcile persisted prefs with current runtime availability. If Codex was
  // toggled on before but is_codex_available() now returns false, clear it so
  // downstream stats consumers see prefs as the single source of truth.
  useEffect(() => {
